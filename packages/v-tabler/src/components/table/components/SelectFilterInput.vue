...
const selectionText = computed(() => {
  ...
  if (selectedValues.value.length === 1) {
    ...
    return props.singleSelectionTextFn ? props.singleSelectionTextFn(displayValue) : displayValue
  }
  
  return props.multipleSelectionTextFn ? props.multipleSelectionTextFn(selectedValues.value.length) : `${selectedValues.value.length} selected`
})
